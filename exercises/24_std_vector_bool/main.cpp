...
int main(int argc, char **argv) {
    ...
    {
        vec[20] = false;
        ASSERT(not vec[20], "Fill in `vec[20]` or `!vec[20]`.");
    }
    {
        vec.push_back(false);
        ASSERT(vec.size() == 101, "Fill in the correct value.");
        ASSERT(not vec[100], "Fill in `vec[100]` or `!vec[100]`.");
    }
    {
        auto ref = vec[30];
        ASSERT(ref, "Fill in `ref` or `!ref`");
        ref = false;
        ASSERT(not ref, "Fill in `ref` or `!ref`");
        // THINK: WHAT and WHY?
        ASSERT(not vec[30], "Fill in `vec[30]` or `!vec[30]`.");
    }
    return 0;
}
